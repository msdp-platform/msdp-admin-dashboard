#!/bin/bash

# Admin Dashboard Fast Development Startup Script
set -e

SERVICE_NAME="admin-dashboard"
CONTAINER_NAME="msdp-admin-dashboard-dev"
PORT=4000

echo "🎛️ Starting MSDP Admin Dashboard (Independent)"
echo "=============================================="

# Check if Docker is running
if ! docker info > /dev/null 2>&1; then
    echo "❌ Docker is not running. Please start Docker first."
    exit 1
fi

# Quick cleanup of existing containers
echo "🧹 Quick cleanup..."
docker-compose -f docker-compose.dev.yml down --remove-orphans > /dev/null 2>&1 || true

# Start services with optimized build
echo "🔨 Starting independent admin dashboard..."
docker-compose -f docker-compose.dev.yml up --build -d --force-recreate

# Wait for database to be ready
echo "⏳ Waiting for database..."
MAX_DB_RETRIES=15
DB_RETRY_COUNT=0

while [ $DB_RETRY_COUNT -lt $MAX_DB_RETRIES ]; do
    if docker exec msdp-admin-postgres-dev pg_isready -U msdp_user -d msdp_admin > /dev/null 2>&1; then
        echo "✅ PostgreSQL is ready!"
        break
    fi
    DB_RETRY_COUNT=$((DB_RETRY_COUNT + 1))
    echo "⏳ Waiting for database... ($DB_RETRY_COUNT/$MAX_DB_RETRIES)"
    sleep 2
done

# Wait for Next.js to be ready
echo "⏳ Waiting for Next.js to compile..."
sleep 10

MAX_RETRIES=15
RETRY_COUNT=0

while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if curl -f http://localhost:$PORT > /dev/null 2>&1; then
        echo "✅ Admin Dashboard is ready!"
        break
    fi
    RETRY_COUNT=$((RETRY_COUNT + 1))
    echo "⏳ Waiting for Next.js... ($RETRY_COUNT/$MAX_RETRIES)"
    sleep 3
done

if [ $RETRY_COUNT -eq $MAX_RETRIES ]; then
    echo "❌ Service failed to start. Check logs with:"
    echo "   docker-compose -f docker-compose.dev.yml logs admin-dashboard"
    exit 1
fi

# Check management tools
sleep 5
if curl -f http://localhost:8086 > /dev/null 2>&1; then
    echo "✅ PgAdmin is ready!"
else
    echo "⏳ PgAdmin still starting..."
fi

echo ""
echo "🎉 Admin Dashboard is ready!"
echo "=============================================="
echo "🎛️ Dashboard: http://localhost:$PORT"
echo "🗄️ PgAdmin: http://localhost:8086 (admin@msdp.com / admin123)"
echo ""
echo "🚀 Admin Features Available:"
echo "  - Location management and analytics"
echo "  - Merchant oversight and approval"
echo "  - User management and roles"
echo "  - System metrics and monitoring"
echo "  - Multi-country administration"
echo ""
echo "🔗 Connected Backend Services:"
echo "  - API Gateway: http://localhost:3000"
echo "  - Location Service: http://localhost:3001"
echo "  - Merchant Service: http://localhost:3002"
echo ""
echo "🔧 Quick commands:"
echo "  logs:    docker-compose -f docker-compose.dev.yml logs -f"
echo "  stop:    docker-compose -f docker-compose.dev.yml down"
echo "  restart: docker-compose -f docker-compose.dev.yml restart $SERVICE_NAME"
echo "  db:      docker exec msdp-admin-postgres-dev psql -U msdp_user -d msdp_admin"
echo ""
